  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (registerUser.fulfilled.match(resultAction)) {
        ...
        localStorage.setItem("login_user", JSON.stringify(basicUserData));

        // تحويل المستخدم للصفحة الرئيسية
        router.push("/");
      } else {
        alert(resultAction.payload || "حدث خطأ أثناء التسجيل");
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
      ...
      <section className="gi-register padding-tb-40">
        <div className="container">
          <div className="section-title-2">
            <h2 className="gi-title">Register<span></span></h2>
                ...
                >
                  {({ handleSubmit, handleChange, values, errors }) => (
                    <Form noValidate onSubmit={handleSubmit}>
                      ...
                      {/* Submit */}
                      <span className="gi-register-wrap gi-register-btn">
                        <span>
